  ...
  <div class="row"><!-- 所有订单之容器 -->
	  ...
	  <div v-for="order in orders" class="col-xs-12 col-sm-12 col-md-6 col-lg-4" style="padding:3px ">
		...
		<div class="row" style="margin:1px 0;padding:0px 18px 0px 18px;background-color:white;">
		    ...
		    <a v-if="param.status=='4refund' && (startWith(order.status,'3') ||  startWith(order.status,'4') || order.status==='54' || order.status==='55' || order.status==='56')" class="btn btn-primary pull-right" :href="'/aftersales/user/refund/begin/' + order.orderId" style="padding:0 3px;margin:0 3px"><span >申请退款</span></a>
		    
		    <a v-if="param.status=='4exchange' && (order.status==='40' || order.status==='41')" class="btn btn-primary pull-right" :href="'/aftersales/user/exchange/begin/' + order.orderId" style="padding:0 3px;margin:0 3px"><span >申请换货</span></a>
...
var containerVue = new Vue({
	...
	methods:{
		getOrders:function(stat,event){
			...
			$.ajax({
				url: '/aftersales/user/getall/' + this.param.status,
				method:'post',
				data: this.param,
